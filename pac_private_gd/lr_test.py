...
def pac_private_logistic_regression(X, y, mu, mi_budget, T=10, privacy_aware=True):
    ...
    for _ in range(T):
        ...
        for d_i in range(d):
            ...
            subset_size = 0
            while subset_size == 0:
                subset = per_sample_grads[np.random.rand(N) < 0.5, d_i]
                subset_size = len(subset)
            grad = subset.mean() + mu * w[d_i] + np.sqrt(C * grad_i_var) * np.random.randn()
        ...
        # print(np.log(y_hat).shape, np.sum(y * np.log(y_hat)))
        loss = -(1/N) * np.sum(y * np.log(y_hat) + (1 - y) * np.log1p(- y_hat))
...
categorical_transformer = Pipeline(steps=[
    ...
    ('scaler', StandardScaler(with_mean=True, with_std=False))  # scale after one-hot encoding
])
...
pipeline = Pipeline(steps=[
    ...
    ('pca', PCA(whiten=True)),  # whiten makes components uncorrelated
])
...
for priv in [True, False]:
    ...
    print(priv)
    for _ in range(10):
        w, losses = pac_private_logistic_regression(X_train, y_train, mu=0.0001, mi_budget=1/256, T=50, privacy_aware=priv)
        acc = evaluate_model(X_test, y_test, w)
        print(acc)
        accs.append(acc)
